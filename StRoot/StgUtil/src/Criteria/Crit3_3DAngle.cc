...
bool Crit3_3DAngle::areCompatible( Segment* parent , Segment* child )throw( BadSegmentLength ){
   ...
   if (( parent->getHits().size() == 2 )&&( child->getHits().size() == 2 )){ //this is a criterion for 2-segments
	 ...
	 //         CritHisto::Record( "Crit3_3DAngle", (acos( sqrt(cosThetaSquared)) * 180. / M_PI), (static_cast<FwdHit*>(a)->_tid == static_cast<FwdHit*>(b)->_tid  && static_cast<FwdHit*>(a)->_tid !=0));
         bool pass = true;
         if (cosThetaSquared < _cosAngleMin*_cosAngleMin) pass = false;
         if (cosThetaSquared > _cosAngleMax*_cosAngleMax) pass = false;

         // if ( false == pass ){
         //    std::cout << "FAIL Crit3_3DAngle = " << acos( sqrt( cosThetaSquared ) ) * 180. / M_PI << std::endl;
         //    std::cout << " Cond : " << _cosAngleMin*_cosAngleMin << " < " << cosThetaSquared << " < " << _cosAngleMax*_cosAngleMax << std::endl;
         // }
         return pass;
