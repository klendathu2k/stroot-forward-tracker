...
#include "streamlog/VerbosityLevels.h"
...
\endverbatim
      ...
      SubsetHopfieldNN(){ 
         ...
         _TStart = 2.1; // temperature parameter
         _TInf = 0.1; // coldest temp
         _omega = 0.75; // how important is quality
         _limitForStable = 0.01; // when to say we are done
         _initStateMin = 0.;
         _initStateMax = 0.1;
   ...
   void SubsetHopfieldNN<T>::calculateBestSet( AreCompatible areCompatible, GetQI getQI ){
      ...
      for ( unsigned i=0; i < nElements ; i++){ //over all elements
         ...
         // states[i] = (_initStateMin + _initStateMax) / 2.0; // TODO : must fix!
         // CLHEP::RandFlat::shoot ( _initStateMin , _initStateMax ); //random ( uniformly ) values from initStateMin to initStateMax
      ...
      if( !elements.empty() ){
         ...
         while ( !net.doIteration() && nIterations < maxIterations){ // while the Neural Net is not (yet) stable
         ...
         if ( nIterations >= maxIterations -1  ){
            cout << "WARNING: HNN Stopped early *****" << endl;
         }
